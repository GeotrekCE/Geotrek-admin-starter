...
tar --exclude='*.djcache'  --exclude='*.tgz' --exclude='*.log' --exclude='*.dump' --exclude='./var/static/*' --exclude='./var/tiles/*' --exclude='./.git/*' -zcvf /var/backups/geotrek/$(basename $PWD)/geotrek_`date +\%Y-\%m-\%d_\%H:\%M`.tgz .
